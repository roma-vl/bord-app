        ...
        <div v-if="props.adverts.data.length" class="divide-y divide-gray-100">
            <div v-for="advert in adverts.data" :key="advert.id"
                ...
                <div class="flex justify-between gap-6 min-h-36">
                    <div class="w-48">
                        <img class="w-full h-40" :src="getFullPathForImage(advert.first_photo?.file)" :alt="advert.title">
                    </div>
                    ...
                    <div class="flex flex-col items-end justify-between">
                        ...
                        <div v-if="props.routes" class="flex items-center gap-3 mt-auto">
                            <a v-if="props.routes.edit" :href="route(props.routes.edit, advert.id)"  title="Редагувати"
                            ...
                            <a v-if="props.routes.show"  :href="route(props.routes.show, advert.id)"
                                class="p-1.5 text-violet-600 hover:text-violet-700 hover:bg-violet-50 rounded-lg transition duration-150 group relative"
                                title="Переглянути">
                                <span class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-1 text-sm text-white bg-gray-800 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">
                                    Переглянути
                            ...
                            <button v-if="props.routes.remove"  @click="remove(advert.id)" class="p-1.5 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition duration-150 group relative"
                                title="Видалити">
                                <span class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-1 text-sm text-white bg-gray-800 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">
                                    Видалити
                                </span>
