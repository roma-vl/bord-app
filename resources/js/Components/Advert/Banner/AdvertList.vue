                ...
                <div class="flex justify-between gap-6 min-h-36">
                    ...
                    <div class="flex-grow flex flex-col justify-between">
                        <div>
                            <a :href="route('account.banners.show', banner.id)" class="block group">
                                <h3 class="text-xl font-semibold text-gray-800 group-hover:text-violet-600 transition-colors duration-200">
                                    {{ banner.name }}
                                </h3>
                            </a>
                        </div>
                        <div class="flex items-center gap-6 text-sm mt-4">
                            ...
                            <p class="text-sm text-gray-500">Опубліковано: {{ getDateFormatFromLocale(banner.created_at )}}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col items-end justify-between">
                        ...
                        <div v-if="props.routes" class="flex items-center gap-3 mt-auto">
                            <a v-if="props.routes.edit" :href="route(props.routes.edit, banner.id)"  title="Редагувати"
                            ...
                            <a v-if="props.routes.show"  :href="route(props.routes.show, banner.id)"
                                class="p-1.5 text-violet-600 hover:text-violet-700 hover:bg-violet-50 rounded-lg transition duration-150 group relative"
                                title="Переглянути">
                                <span class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-1 text-sm text-white bg-gray-800 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">
                                    Переглянути
                            ...
                            <button v-if="props.routes.remove"  @click="remove(banner.id)" class="p-1.5 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition duration-150 group relative"
                                title="Видалити">
                                <span class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-1 text-sm text-white bg-gray-800 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">
                                    Видалити
                                </span>
