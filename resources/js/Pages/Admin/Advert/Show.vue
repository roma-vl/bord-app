...
const openCreateModal = async () => {
    const { data } = await axios.get(route("admin.adverts.category.attributes.create",  props.category.id ));
...
const openEditModal = async (attributeId) => {
    const { data } = await axios.get(route("admin.adverts.category.attributes.edit", {
...
const refreshAttributes = () => {
    router.get(route("admin.adverts.category.show", {category: props.category.id}), {
        preserveScroll: true,
        onSuccess: () => router.replace(route("admin.adverts.category.show", {category: props.category.id})),
    });
};

const deleteCategory = (attributeId) => {
    if (confirm("Are you sure you want to delete this Category?")) {
        router.delete(route("admin.adverts.category.attributes.destroy", {
            category: props.category.id,
            attribute: attributeId
        }), {
            preserveScroll: true,
                ...
                <div class="min-w-full bg-white rounded-lg shadow p-6 min-h-[700px]">
                    ...
                    <table class="w-full mt-4 border-collapse border border-gray-200">
                        ...
                        <tr v-for="attr in attributes" :key="attr.id">
                            ...
                            <td class="border p-2">{{ attr.sort }}</td>
                            <td class="border p-2">
                                <button @click.stop="openEditModal(attr.id)" class="text-blue-500 pr-2 hover:underline">Редагувати</button>
                                <button @click.stop="deleteCategory(attr.id)" class="text-red-500 hover:underline">Видалити</button>
                            </td>
                        </tr>
                        </tbody>
                    </table>
